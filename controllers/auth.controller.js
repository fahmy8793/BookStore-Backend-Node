...
const register = async (req, res) => {
  ...
  try {
    ...
    if (pendingExists) {
      return res
        .status(400)
        .json({ message: "Please verify the OTP sent to your email" });
    }
...
const sendOTP = async (req, res) => {
  ...
  try {
    ...
    if (
      user.otpLastSentAt &&
      Date.now() - user.otpLastSentAt.getTime() < 10 * 60 * 1000
    ) {
      return res.status(429).json({
  ...
  try {
    ...
    if (existingUser && !existingUser.isGoogleUser) {
      return res.status(400).json({
        message:
          "Email already registered with password. Please login manually.",
      });
    }
